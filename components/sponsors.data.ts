	...
	async load(): Promise<Sponsor[]> {
		try {
			...
			for (const sponsor of data) {
				if (sponsor.tier.monthlyPriceInDollars >= 20) {
					sponsor.tier.name = 'gold'
				} else if (sponsor.tier.monthlyPriceInDollars >= 10) {
					sponsor.tier.name = 'silver'
				} else {
					sponsor.tier.name = 'any'
				}
			}
			return data
				.filter(
